...
<div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
    ...
    <div class="bg-white rounded-xl shadow p-6">
        <h2 class="text-xl font-bold text-slate-900 mb-4">Income vs Expenses</h2>
        <canvas id="incomeExpensesChart" height="300"></canvas>
    </div>

    <!-- Budget Distribution Pie Chart -->
    <div class="bg-white rounded-xl shadow p-6">
        <h2 class="text-xl font-bold text-slate-900 mb-4">Budget Allocation</h2>
...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
    ...
    <div class="bg-white rounded-xl shadow p-6">
        ...
        <div class="mt-4 h-2 bg-slate-200 rounded-full">
            <div class="h-2 bg-teal-600 rounded-full" style="width: {{ ($stats['savingsRate'] * 100) }}%"></div>
        </div>
    </div>
    ...
    <div class="bg-white rounded-xl shadow p-6">
        ...
        <div class="mt-4 h-2 bg-slate-200 rounded-full">
            <div class="h-2 bg-purple-600 rounded-full" style="width: {{ ($stats['givingRate'] * 100) }}%"></div>
        </div>
    </div>
</div>
...
<script>
    // Income vs Expenses Chart
    const incomeCtx = document.getElementById('incomeExpensesChart').getContext('2d');
    new Chart(incomeCtx, {
        type: 'bar',
        data: {
    ...
    new Chart(budgetCtx, {
        type: 'doughnut',
        data: {
            labels: ['Expenses', 'Savings', 'Giving', 'Remaining'],
            datasets: [{
                data: [
                    {{ $stats['totalExp'] }},
                    {{ $stats['totalSav'] }},
                ...
                backgroundColor: ['#ef4444', '#10b981', '#7b1fa2', '#3b82f6'],
                borderRadius: 8,
            }]
        },
        options: {
            ...
            plugins: {
                ...
                tooltip: { callbacks: { label: function(context) { return 'GHS ' + context.parsed.toLocaleString(); } } }
            }
        }
    });
</script>
