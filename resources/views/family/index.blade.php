...
<div class="mb-8 flex justify-between items-center">
    <div>
        <h1 class="text-4xl font-bold text-slate-900 mb-2">👨‍👩‍👧 Family Support</h1>
        <p class="text-slate-600">Track family assistance and transfers</p>
    </div>
    ...
    <table class="w-full">
        <thead class="bg-slate-50 border-b border-slate-200">
            <tr>
                ...
                <th class="px-6 py-4 text-left text-sm font-semibold text-slate-900">Relationship</th>
        ...
        <tbody class="divide-y divide-slate-200">
            @forelse($entries as $entry)
                <tr class="hover:bg-slate-50">
                    ...
                    <td class="px-6 py-4 text-sm"><span class="px-3 py-1 bg-pink-100 text-pink-800 rounded-full text-xs font-semibold">{{ $entry->relationship }}</span></td>
                    <td class="px-6 py-4 text-sm font-bold text-pink-600">GHS {{ number_format($entry->amount, 2) }}</td>
            ...
            @empty
                <tr><td colspan="5" class="px-6 py-8 text-center text-slate-500">No family support records yet.</td></tr>
            @endforelse
        </tbody>
    </table>
</div>
